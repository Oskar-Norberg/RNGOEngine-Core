...
{
    RenderAPI::RenderAPI(
        IRenderer& renderer, Resources::ResourceTracker& resourceTracker,
        const Resources::ResourceManager& resourceManager, const AssetHandling::ModelManager& modelManager,
    ...
    void RenderAPI::RenderOpaque(Window::IWindow& window) const
    {
        ...
        for (const auto& opaqueDrawCall : m_drawQueue.opaqueObjects)
        {
            ...
            // And again, set per object. Terrible.
            {
                ...
                m_renderer.SetFloat(
                    shaderProgramID, "ambientLight.intensity", m_drawQueue.ambientLight.intensity
                );
                ...
                m_renderer.SetFloat(
                    shaderProgramID, "directionalLight.intensity", m_drawQueue.directionalLight.intensity
                );
                ...
                for (size_t i = 0; i < m_drawQueue.pointLightIndex; i++)
                {
                    ...
                    m_renderer.SetFloat(
                        shaderProgramID, intensity.c_str(), m_drawQueue.pointLights[i].intensity
                    );
                    ...
                    m_renderer.SetFloat(
                        shaderProgramID, constant.c_str(), m_drawQueue.pointLights[i].constant
                    );
                    ...
                    m_renderer.SetFloat(
                        shaderProgramID, quadratic.c_str(), m_drawQueue.pointLights[i].quadratic
                    );
                }
                m_renderer.SetInt(
                    shaderProgramID, "numPointLights", static_cast<int>(m_drawQueue.pointLightIndex)
                );
                ...
                for (size_t i = 0; i < m_drawQueue.spotlightIndex; i++)
                {
                    ...
                    m_renderer.SetFloat(
                        shaderProgramID, intensity.c_str(), m_drawQueue.spotlights[i].intensity
                    );
                    ...
                    m_renderer.SetFloat(
                        shaderProgramID, outerCutoff.c_str(), m_drawQueue.spotlights[i].outerCutoff
                    );

                    const auto constant = std::format("{}.constant", spotlightBase);
                    m_renderer.SetFloat(
                        shaderProgramID, constant.c_str(), m_drawQueue.spotlights[i].constant
                    );
                    ...
                    m_renderer.SetFloat(
                        shaderProgramID, quadratic.c_str(), m_drawQueue.spotlights[i].quadratic
                    );
                }
                m_renderer.SetInt(
                    shaderProgramID, "numSpotlights", static_cast<int>(m_drawQueue.spotlightIndex)
                );
            }
            ...
            {
                std::visit(
                    [this, shaderProgramID, &name]<typename T0>(T0&& arg)
                    {
